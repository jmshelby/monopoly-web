(ns jmshelby.monopoly-web.player-templates
  (:require [clojure.string :as str]
            [cljs.pprint :refer [pprint]]
            [clojure.edn :as edn]))

(def dumb-player-edn 
  "EDN structure representing the dumb player template"
  ;; We'll inline this rather than reading from file for ClojureScript
  {:namespace-name "my-custom-player"
   :requires [['clojure.set :as 'set]
              ['jmshelby.monopoly.util :as 'util]]
   :functions
   [;; Private helper function
    {:name "percent-props-owned"
     :private true
     :args ['game-state]
     :body '(let [total (->> game-state :board :properties
                             (map :name) count)
                  owned (->> game-state
    ...
    ;; Public functions  
    {:name "trade-side-value"
     :doc "Given a game board, player, and resources from one
  side of a trade proposal, return the total \"real\"
  value of all resources."
     :args ['board 'player 'resources]
     :body '(letfn [(prop-def [id]
                      (->> board :properties
                           (filter #(= id (:name %)))
                           first))]
              (->> resources
    ...
    {:name "proposal-benefit"
     :doc "Given a game-state and proposal map, return the ratio
  ...
  subtract 10% for real value)."
     :args ['game-state 'proposal]
     :body '(let [board       (:board game-state)
    ...
    {:name "find-desired-properties"
     :args ['game-state 'player]
     :body '(let [player-id    (:id player)
    ...
    {:name "find-proposable-properties"
     :args ['game-state 'player 'target-value]
     :body '(let [player-id    (:id player)
                  group->count (util/street-group-counts (:board game-state))
                  ;; Map group name -> set of prop names
                  owned-props  (util/owned-property-details game-state)]
    ...
    {:name "proposal?"
     :doc "Given a game-state and player, return the best current
  proposal available, _if_ it's a smart time to propose one."
     :args ['game-state 'player]
     :body '(let [;; Get a possible single desired property
    ...
    {:name "decide"
     :doc "Main player decision function"
     :args ['game-state 'player-id 'method 'params]
     :body '(let [{my-id :id
                   cash  :cash
                   :as   player}
...
(defn- format-require [req]
  (if (= 2 (count req))
    (str "[" (first req) " :as " (second req) "]")
    (str "[" (first req) "]")))

(defn- format-function-def [func]
  (let [{:keys [name args body doc private]} func
        fn-symbol (if private (str "defn- " name) (str "defn " name))
        docstring (if doc (str "\n  \"" doc "\"") "")
        args-str (str "[" (str/join " " args) "]")]
    (str "(" fn-symbol docstring "\n  " args-str "\n  " 
         (with-out-str (pprint body)) ")")))

(defn generate-player-code [edn-template]
  (let [{:keys [namespace-name requires functions]} edn-template
        ns-clause (str "(ns " namespace-name "\n  (:require " 
                       (str/join "\n            " (map format-require requires)) "))\n\n")
        function-defs (str/join "\n\n" (map format-function-def functions))]
    (str ns-clause function-defs)))

(def dumb-player-template
  (generate-player-code dumb-player-edn))